...
- [6. 코인 디스커버 (AI 추천)](#6-코인-디스커버-ai-추천)
- [7. 서버 상태 확인](#7-서버-상태-확인)
- [데이터 타입 참조](#데이터-타입-참조)
...
```json
{
  ...
  "strategyAdvice": {
    "shortTerm": {
      ...
      "tp": 67500,
      "sl": 62800,
      "outlook": "단기적으로는 $65,000 지지 여부가 가장 중요합니다.",
      "keyLevel": "$65,000 지지선, $67,500 저항선",
      "tip": "$66,000 돌파 시 본절가에 스탑로스를 옮겨 무위험 포지션으로 전환하세요."
    },
    "midTerm": {
      "period": "8일~21일",
      "tp": 72000,
      "sl": 60500,
      "outlook": "중기적으로는 주봉 단위의 흐름을 봅니다. $65k 위에서 안착 시 새로운 가격 발견 단계 진입.",
      "keyLevel": "$60,500 중기 지지선, $72,000 피보나치 확장 타겟",
      "tip": "펀딩비가 지나치게 높지 않은지 주기적으로 체크하세요."
    }
  }
}
```
...
시장 전체 데이터를 분석하여 AI가 트레이딩 코인을 추천한다. Hyperliquid 전 종목의 가격 변동률, 거래량, 펀딩 레이트, OI를 기반으로 추천 코인, 방향(LONG/SHORT), TP/SL, 신뢰도를 제공한다.

### `POST /api/v1/analysis/discover`

#### Request Body

별도 파라미터 없음 (빈 body 또는 `{}`).
  ...
  "recommendations": [
    {
      ...
      "direction": "LONG",
      "currentPrice": 1.58,
      "changePercent24h": 10.2,
      "tp": 1.65,
      "sl": 1.48,
      "confidence": "high",
      "reason": "24시간 거래량 1위, 상승 깃발형 패턴. 전고점 돌파 시 숏 스퀴즈 가능성."
    },
    {
      "coin": "HYPE",
      "direction": "LONG",
      ...
      "sl": 39.8,
      "confidence": "medium",
      "reason": "라운드 피겨 $40 위에서 강력한 지지. 1시간봉 기준 저점 상승 추세."
    }
  ]
}
```
...
| 필드 | 타입 | 설명 |
|------|------|------|
| recommendations | DiscoverRecommendation[] | AI 추천 코인 목록 |
| recommendations[].coin | string | 코인 심볼 |
| recommendations[].direction | "LONG" \| "SHORT" | 추천 방향 |
| recommendations[].currentPrice | number | 현재 가격 |
| recommendations[].changePercent24h | number | 24시간 변동률 (%) |
| recommendations[].tp | number | Take Profit 가격 |
| recommendations[].sl | number | Stop Loss 가격 |
| recommendations[].confidence | "high" \| "medium" \| "low" | 신뢰도 |
| recommendations[].reason | string | 추천 사유 (한국어) |
...
| 필드 | 타입 | 설명 |
|------|------|------|
| shortTerm | StrategyTimeframe | 단기 전략 (1일~7일) |
| midTerm | StrategyTimeframe | 중기 전략 (8일~21일) |

### StrategyTimeframe

| 필드 | 타입 | 설명 |
|------|------|------|
| period | string | 기간 (e.g. "1일~7일") |
...
|------|------|------|
| coin | string | 코인 심볼 |
| direction | string | 추천 방향 ("LONG" \| "SHORT") |
| currentPrice | number | 현재 가격 |
| changePercent24h | number | 24시간 변동률 (%) |
| tp | number | Take Profit 가격 |
| sl | number | Stop Loss 가격 |
| confidence | string | 신뢰도 ("high" \| "medium" \| "low") |
